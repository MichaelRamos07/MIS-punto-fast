...
void cargarUsuarios();
void registrarOActualizarCliente(char cedula[], float montoCompra);
...
void venderCarrito() {
    ...
    guardarProductos();
    registrarOActualizarCliente(fd.cedulacliente, total);
...
void agregarVendedor(Usuario u) {
    ...
    if (cantidadUsuarios == 0) {
        ...
        u.id = usuarios[cantidadUsuarios - 1].id + 1;
    }
...
void registrarOActualizarCliente(char cedula[], float montoCompra) {
    ...
    printf("Inicio registro:\n"); // Tu print de control

    if (f != NULL) {
        fgets(linea, sizeof(linea), f); // Saltar el encabezado "cedula,nombre..."
        
        // 1. EL BUCLE CORRECTO: Lee todo el archivo y lo guarda en el arreglo
        while (fgets(linea, sizeof(linea), f)) {
            Cliente c;
            char *token = strtok(linea, ",");
            if (token == NULL) continue;
            
            strcpy(c.cedula, token);
            
            token = strtok(NULL, ",");
            if (token) strcpy(c.nombre, token);
            
            token = strtok(NULL, ",");
            if (token) c.totalGastado = atof(token);
            
            token = strtok(NULL, ",");
            if (token) c.puntos = atoi(token);
            
            // Si la cédula coincide, actualizamos sus datos
            if (strcmp(c.cedula, cedula) == 0) {
                c.totalGastado += montoCompra;
                c.puntos += (int)montoCompra; 
                encontrado = 1;
                printf("\n[Cliente Frecuente] %s acumulo %d puntos! Total: %d\n", c.nombre, (int)montoCompra, c.puntos);
            }
            
            // Guardamos el cliente en nuestro arreglo temporal
            listaClientes[cantClientes++] = c;
        }
        fclose(f);
    }

    // 2. FUERA DEL IF DEL ARCHIVO: Si nadie coincidió, es un cliente nuevo
    if (!encontrado) {
        Cliente nuevoCliente;
        strcpy(nuevoCliente.cedula, cedula);
        
        printf("\n[Cliente Nuevo] Ingrese el nombre del cliente: ");
        fgets(nuevoCliente.nombre, sizeof(nuevoCliente.nombre), stdin);
        nuevoCliente.nombre[strcspn(nuevoCliente.nombre, "\n")] = '\0';
        
        nuevoCliente.totalGastado = montoCompra;
        nuevoCliente.puntos = (int)montoCompra;
        
        listaClientes[cantClientes++] = nuevoCliente;
        printf("¡Cliente registrado exitosamente con %d puntos!\n", nuevoCliente.puntos);
    }

    f = fopen("clientes.csv", "w");
    if (f == NULL) {
        printf("Error al abrir clientes.csv para escribir.\n");
        return;
    }
    
    fprintf(f, "cedula,nombre,totalGastado,puntos\n");
    for (int i = 0; i < cantClientes; i++) {
        fprintf(f, "%s,%s,%.2f,%d\n", 
                listaClientes[i].cedula, 
                listaClientes[i].nombre, 
                listaClientes[i].totalGastado, 
                listaClientes[i].puntos);
    }
    fclose(f);
}
